...
namespace BotwActorToolLib.MSBT.Sections
{
    ...
    internal class Label : ICalculatesSize
    {
        ...
        [NotNull]
        public Lbl1 lbl1;
        public string name { get => p_name; set { p_name = value; checksum = GenerateChecksum(value); } }
        public uint index;
        public uint checksum { get; private set; }
        public Label() { lbl1 = new(); }
        ...
        public static uint GenerateChecksum(string name)
        {
            unchecked
            {
                return Encoding.UTF8.GetBytes(name).ToList().Aggregate(0u, (hash, b) => hash * HASH_MAGIC + b) % lbl1.group_count;
            }
        }
